...
function classifyChordFilter(name) {
  const suffix = getChordSuffix(name);
  if (['', 'maj', '5', '6', '6/9', 'add9', 'add11', 'add13', 'aug'].includes(suffix)) return 'major';
  if (['m', 'm6', 'mMaj7'].includes(suffix)) return 'minor';
  if (['7', '9', '11', '13', '7b5', '7#5', '7b9', '7#9'].includes(suffix)) return 'dominant';
  if (['maj7', 'maj9'].includes(suffix)) return 'maj7';
  if (['m7', 'm9', 'm11'].includes(suffix)) return 'm7';
  if (suffix === 'sus2' || suffix === 'sus4' || suffix === 'sus') return 'sus';
  if (suffix === 'dim' || suffix === 'dim7' || suffix === 'm7b5') return 'dim';
  return 'other';
}

function passesChordFilter(name) {
  return state.chordFilter === 'all' || classifyChordFilter(name) === state.chordFilter;
}
...
function getPreviewMidiNotesForChord(chord) {
  ...
  const bassMidi = Math.max(24, upperVoicing[0] - 12);
...
async function previewGroupChord(name, button = null) {
  ...
  engine.previewChord(midiNotes, 94, {
    attackTime: 0.022,
    holdTime: 0.42,
    releaseTime: 0.5,
    noteGain: 0.24
  });
  window.setTimeout(() => button?.classList.remove('is-playing'), 520);
  return true;
}
...
async function previewChordNotes(notes, surface = null) {
  ...
  window.setTimeout(() => surface?.classList.remove('is-playing'), 520);
  return true;
}
...
function attachChordPreviewSurface(surface, chord) {
  ...
  surface.title = `Прослушать ${chord.name}`;
...
function updateGroupsToggleButtonState() {
  ...
  if (!button) return;
  button.textContent = hasAnyExpandedGroups() ? 'Свернуть все группы' : 'Развернуть все группы';
}
...
function renderChordsList(prevSearchLen = state.lastSearchLen) {
  ...
  for (const root of orderedRoots) {
    ...
    items.forEach(name => {
      ...
      playButton.setAttribute('aria-label', `Прослушать ${name}`);
      playButton.title = `Прослушать ${name}`;
...
function addSongTextOccurrencesToSelected() {
  ...
  if (currentSignature && currentSignature === songTextState.lastAddedSignature) {
    showAppToast('Аккорды уже добавлены');
    return;
  }
  songTextState.occurrences.forEach(occ => {
  ...
  renderAll();
  showAppToast(songTextState.occurrences.length ? 'Аккорды добавлены' : 'Аккорды не найдены');
}
...
function init() {
  ...
  el('saveSong').addEventListener('click', () => {
    ...
    if (!name) { showAppToast('Введите название песни'); return; }
    saveSong(name);
    el('songName').value = '';
    showAppToast('Песня сохранена');
  });
  ...
  el('songTextAddSaved').addEventListener('click', () => {
    addSongTextOccurrencesToSelected();
  });
  el('songTextSave').addEventListener('click', () => {
    const title = (el('songTextTitle').value || '').trim();
    if (!title) { showAppToast('Введите название песни'); return; }
